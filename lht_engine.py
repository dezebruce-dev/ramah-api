...
class LHTDiagnosticEngine:
    ...
    def telos_run(self, reference: str, text: str) -> TelosRun:
        ...
        # 5. GRAVITY-SYNC
        gravity = self.gravity_sync(text, seeds, spiral, quadrants)
    ...
    def assign_spiral(self, reference: str) -> Spiral:
        ...
        Uses deterministic book-to-spiral mapping per LHT canonical model.
        No heuristics - just clean lookup table.
        
        Returns:
            Spiral enum (1-7) based on book name in reference
        """
        ref_lower = reference.lower()
        
        # Spiral 1: Genesis 1-11 (Creation)
        if 'genesis' in ref_lower:
            # Check chapter for Genesis 1-11 vs 12+
        ...
        # Spiral 2: Genesis 12 - Exodus (Covenant) - pre-Sinai
        if 'exodus' in ref_lower:
            return Spiral.S2_COVENANT
        
        # Spiral 3: Exodus (Sinai) - Deuteronomy (Law & Wilderness)
        if any(book in ref_lower for book in ['leviticus', 'numbers', 'deuteronomy']):
            return Spiral.S3_LAW
        
        # Spiral 4: Joshua - 2 Kings (Kingship)
        # Includes Chronicles as parallel
        if any(book in ref_lower for book in [
            'joshua', 'judges', 'ruth', 
            '1 samuel', '2 samuel', 'samuel',
            '1 kings', '2 kings', 'kings',
            '1 chronicles', '2 chronicles', 'chronicles'
        ]):
            return Spiral.S4_KINGSHIP
        
        # Spiral 5: Job - Malachi (Wisdom & Prophets)
        if any(book in ref_lower for book in [
            'job', 'psalm', 'proverb', 'ecclesiastes', 'song of solomon', 'song',
        ...
        # Exclude epistles of John
        if any(book in ref_lower for book in ['matthew', 'mark', 'luke']):
            return Spiral.S6_CHRIST
        if 'john' in ref_lower and not any(epistle in ref_lower for epistle in ['1 john', '2 john', '3 john']):
            return Spiral.S6_CHRIST
        
        # Spiral 7: Acts - Revelation (Church & New Creation)
        if any(book in ref_lower for book in [
            'acts', 'romans', 
            '1 corinthians', '2 corinthians', 'corinthians',
            'galatians', 'ephesians', 'philippians', 'colossians',
            '1 thessalonians', '2 thessalonians', 'thessalonians',
            '1 timothy', '2 timothy', 'timothy', 'titus', 'philemon',
            'hebrews', 'james',
            '1 peter', '2 peter', 'peter',
            '1 john', '2 john', '3 john',  # Epistles of John
            'jude', 'revelation'
        ]):
            return Spiral.S7_NEW_CREATION
        
        # Default: assume Gospel context if completely unknown
        return Spiral.S6_CHRIST
    ...
    def gravity_sync(self, text: str, seeds: List[PatternSeed], 
                    spiral: Spiral = None, quadrants: 'QuadrantMapping' = None) -> GravitySync:
        """
        Calculate Christ-centrality score (0-3).
        
        Scoring per LHT specification:
        3 = Direct Christ event / Cross pattern (Gospels with crucifixion/resurrection)
        2 = Strong typology (OT sacrifice, SE→NE→NW movement, blood/lamb/altar)
        1 = Weak/implicit echo (death→life, exile→return, general symbols)
        0 = No discernible Christ gravity
        
        Args:
            text: Passage text
            seeds: Pattern seeds identified
            spiral: Current spiral context (optional)
            quadrants: Quadrant mapping (optional)
        """
        text_lower = text.lower()
        christ_patterns = []
        
        # Score 3: Direct Christ event
        # - Explicit Gospel crucifixion/resurrection language
        # - Direct NT interpretation of the Cross
        direct_christ_words = ['crucif', 'cross', 'finished', 'resurrection', 'risen', 
                              'golgotha', 'calvary', 'tomb', 'third day']
        for word in direct_christ_words:
            if word in text_lower:
                christ_patterns.append(word)
        
        # Also check if in Gospels with Christ-event language
        if spiral and spiral == Spiral.S6_CHRIST and christ_patterns:
            return GravitySync(
                score=3,
                explanation="Direct Christ event in Gospels",
        ...
        # Score 2: Strong typology
        # - Clear substitutionary/sacrificial structure
        # - OT passages with SE→NE→NW (fall → revelation → restoration)
        # - Multiple strong symbols (BLOOD, LAMB, ALTAR, PRIEST)
        strong_type_symbols = ['lamb', 'blood', 'sacrifice', 'altar', 'priest', 
                              'atonement', 'scapegoat', 'passover']
        symbol_count = 0
        for seed in seeds:
            if seed.symbol.lower() in strong_type_symbols:
                christ_patterns.append(seed.symbol)
                symbol_count += 1
        
        # Check for SE→NE→NW movement (fall → revelation → restoration)
        has_redemptive_arc = False
        if quadrants and quadrants.movement:
            movement = quadrants.movement.upper()
            # Look for patterns like SE→NE→NW or SW→NE→NW
            if ('SE' in movement or 'SW' in movement) and 'NE' in movement and 'NW' in movement:
                has_redemptive_arc = True
        
        # Strong typology if: multiple symbols OR symbols + redemptive arc
        if symbol_count >= 2 or (symbol_count >= 1 and has_redemptive_arc):
            explanation = "Strong sacrificial typology"
            if has_redemptive_arc:
                explanation += " with redemptive arc (fall→revelation→restoration)"
            return GravitySync(
                score=2,
        ...
        # Score 1: Weak/implicit hints
        # - General messianic symbols without clear sacrifice
        # - Death→life patterns
        # - Exile→return narratives
        weak_hint_symbols = ['seed', 'crown', 'king', 'son', 'shepherd', 
                            'tree', 'water', 'bread', 'light']
        for seed in seeds:
            if seed.symbol.lower() in weak_hint_symbols:
                christ_patterns.append(seed.symbol)
        
        # Also check for death→life or exile→return arcs
        has_weak_arc = False
        if quadrants and quadrants.movement:
            movement = quadrants.movement.upper()
            # SW→NW or SE→NW without explicit NE (death→life without teaching)
            if ('SW' in movement or 'SE' in movement) and 'NW' in movement:
                has_weak_arc = True
        
        if christ_patterns or has_weak_arc:
            explanation = "Light messianic hints"
            if has_weak_arc:
                explanation += " with narrative arc toward restoration"
            return GravitySync(
                score=1,
                explanation=explanation,
                christ_patterns=christ_patterns
        ...
        # Score 0: No discernible Christ gravity
        return GravitySync(
            score=0,
    ...
    def comp_435(self, text: str) -> Comp435:
        ...
        The 4→3→5 pattern represents the canonical movement from:
        Stage 4 (CHAOS): World / lack / toil / suffering / "no fish in the net"
        Stage 3 (PATTERN): Divine word / instruction / command / "cast on the right side"  
        Stage 5 (GRACE): Overflow / abundance / rest / restoration / "153 fish + breakfast"
        
        Examples:
        - Creation: formless void (4) → "Let there be light" (3) → filled earth (5)
        - John 21: empty nets (4) → Christ's command (3) → 153 fish (5)
        - Cross: death/chaos (4) → "It is finished" (3) → resurrection (5)
        
        Returns:
            Comp435 with has_pattern, mapping description, confidence level, and stage detection
        """
        text_lower = text.lower()
        
        # Stage 4: Chaos / lack / toil / suffering
        # Words indicating deficit, emptiness, labor without result
        stage_4_words = [
            'without', 'empty', 'nothing', 'void', 'darkness', 'lack',
            'toil', 'labor', 'weary', 'barren', 'desert', 'waste',
            'night', 'chaos', 'formless', 'hungry', 'thirsty'
        ]
        has_stage_4 = any(w in text_lower for w in stage_4_words)
        
        # Stage 3: Divine pattern / word / instruction / command
        # Words indicating God's intervention, revelation, direction
        stage_3_words = [
            'god said', 'lord said', 'thus saith', 'commanded', 'spoke',
            'word', 'behold', 'hear', 'listen', 'law', 'instruction',
            'teach', 'revealed', 'prophesy'
        ]
        has_stage_3 = any(w in text_lower for w in stage_3_words)
        
        # Stage 5: Grace / overflow / abundance / rest / restoration
        # Words indicating blessing, multiplication, completion, peace
        stage_5_words = [
            'filled', 'full', 'overflow', 'abundance', 'multiply', 'multiplied',
            'blessed', 'blessing', 'feast', 'rest', 'sabbath', 'peace',
            'rejoice', 'joy', 'new', 'renewed', 'restored', 'gathered'
        ]
        has_stage_5 = any(w in text_lower for w in stage_5_words)
        
        # Pattern detected if all three stages present
        has_pattern = has_stage_4 and has_stage_3 and has_stage_5
        
        if has_pattern:
            mapping = "4: chaos/lack → 3: divine word/instruction → 5: grace/overflow/abundance"
            
            # Determine confidence based on how clearly the stages appear
            stage_count = sum([has_stage_4, has_stage_3, has_stage_5])
            if stage_count == 3:
                # Check if stages appear in reasonable sequence
                # (simplified - production would analyze actual ordering)
                confidence = "Medium"
            else:
                confidence = "Low"
                
            stages = {
                "stage_4_chaos": has_stage_4,
                "stage_3_pattern": has_stage_3,
                "stage_5_grace": has_stage_5
            }
        else:
            mapping = None
            confidence = "Low"
    ...
    def generate_summary(self, spiral: Spiral, quadrants: QuadrantMapping,
        ...
        Never crashes even with empty or None inputs.
        Produces clean, readable English following LHT structure.
        """
        # Safe extraction of spiral name
        spiral_name = spiral.name if spiral else "Unknown Spiral"
        
        # Safe extraction of movement
        movement = quadrants.movement if quadrants and quadrants.movement else "Unknown movement"
        
        # Safe extraction of seed names
        seed_names = []
        if seeds:
            seed_names = [s.symbol for s in seeds[:3] if s and s.symbol]
        
        # Build summary safely
        summary = f"This passage sits in {spiral_name}, "
        summary += f"moving through {movement}. "
        
        if seed_names:
            summary += f"The dominant symbols are {', '.join(seed_names)}. "
        else:
            summary += "No dominant symbols were identified. "
        
        # Add Christ-centered interpretation based on gravity score
        if gravity and gravity.score == 3:
            summary += "This directly reveals Christ's redemptive work."
        elif gravity and gravity.score == 2:
            summary += "This strongly foreshadows the Cross pattern through sacrificial typology."
        elif gravity and gravity.score == 1:
